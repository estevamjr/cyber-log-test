...
describe('LogService', () => {
  ...
  it('deve calcular corretamente o maior kill streak de cada jogador na partida', async () => {
    const logContent = `
      ...
      12:04:00 - PlayerA killed PlayerB using AK-47
      12:05:00 - PlayerA killed PlayerC using AK-47
      12:06:00 - PlayerA killed PlayerB using AK-47
  ...
  describe('getMVReport', () => {
    it('deve retornar mensagem se nenhuma partida for encontrada', async () => {
      mockLogRepository.getAllMatchReports.mockResolvedValue([]);
      const result = await logService.getMVReport();
      expect(result).toEqual({ message: 'Nenhuma partida encontrada para gerar o relatório MVP.' });
    });

    it('deve calcular o MVP corretamente com prêmios', async () => {
      const mockSavedMatches = [
        {
          id: 1,
          matchId: '100',
          report: {
            awards: [
              { player: 'SuperStar', award: 'KILLING_SPREE' },
              { player: 'SuperStar', award: 'FLAWLESS_VICTORY' }
            ]
          }
        },
        {
          ...
          report: {
            awards: [
              { player: 'PlayerF', award: 'FLAWLESS_VICTORY' }
            ]
          }
        },
      ];
      mockLogRepository.getAllMatchReports.mockResolvedValue(mockSavedMatches);

      const result = await logService.getMVReport();

      expect(result).toEqual([
        { player: 'SuperStar', score: 8, awards_summary: { 'KILLING_SPREE': 1, 'FLAWLESS_VICTORY': 1 } },
        { player: 'PlayerF', score: 5, awards_summary: { 'FLAWLESS_VICTORY': 1 } },
      ]);
    });

    it('deve retornar mensagem se nenhum jogador tiver prêmios', async () => {
      const mockSavedMatches = [
        { id: 1, matchId: '100', report: { awards: [] } }
      ];
      mockLogRepository.getAllMatchReports.mockResolvedValue(mockSavedMatches);

      const result = await logService.getMVReport();
      expect(result).toEqual({ message: 'Não foi possível determinar um MVP claro (nenhum jogador com prêmios).' });
    });
  });
});